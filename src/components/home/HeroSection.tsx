...
const HeroSection = () => {
  ...
  return (
    <section id="home" className="relative min-h-[90vh] flex items-center bg-gradient-to-r from-purple-900 via-purple-800 to-amber-900 text-white overflow-hidden">
      ...
      ></div>
      <div 
        className={`absolute left-20 bottom-10 w-80 h-80 bg-purple-500/20 rounded-full blur-3xl transition-all duration-1500 ease-in-out ${playAnimation ? 'scale-125 opacity-80' : 'opacity-30'}`}
        style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className={`transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#join" className="transform transition-transform duration-300 hover:scale-105">
                <Button className="bg-amber-500 hover:bg-amber-600 text-white px-6 py-2 rounded-full shadow-lg transition-all duration-300">
                  Join Now <ChevronRight className="h-4 w-4 ml-1" />
                </Button>
              </a>
              <a href="#courses" className="transform transition-transform duration-300 hover:scale-105">
                <Button variant="outline" className="bg-transparent hover:bg-white/10 text-white border-white px-6 py-2 rounded-full transition-colors duration-300">
                  <BookOpen className="h-4 w-4 mr-2" /> Explore Courses
                </Button>
              </a>
              <Button 
                variant="ghost" 
                className="bg-purple-700/50 hover:bg-purple-700/70 text-white rounded-full transform transition-all duration-300 hover:scale-105 hover:shadow-purple-500/30 hover:shadow-lg"
              >
          ...
          <div className={`flex justify-center transition-all duration-1000 delay-300 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <div className="relative">
              ...
              ></div>
              <div 
                className="absolute -bottom-6 -left-6 w-32 h-32 bg-purple-500 rounded-full opacity-30 blur-md"
